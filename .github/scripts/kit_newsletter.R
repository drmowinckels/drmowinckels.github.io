send_newsletter <- function(frontmatter, url, body = NULL) {
  body_text <- body %||% frontmatter$summary
  ...
  body_html <- body_text |>
    ...
    sapply(function(p) {
      sprintf(
        '<p class="ck-paragraph" style="margin:1em 0;font-size:18px;line-height:1.5em;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,Oxygen-Sans,Ubuntu,Cantarell,\'Helvetica Neue\',sans-serif;color:#0c4848;font-size:16px;line-height:1.5">%s</p>',
        p
      )
    }) |>
    paste(collapse = "\n")
  ...
  httr2::request("https://api.convertkit.com/v3/broadcasts") |>
    httr2::req_body_json(
      list(
        ...
        content = newsletter,
        send_at = format(Sys.time(), "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
      )
    ) |>
    httr2::req_perform()
}
