...
do.call(bind_rows,
  ...
  detailed_entries(
    with = glue::glue("{`education-summary.role-title`} - {`education-summary.organization.name`} - Norway"),
...
scholar::get_publications("7NkxgWQAAAAJ") %>%
  ...
  mutate(
    ...
    cid = glue::glue("https://scholar.google.no/scholar?oi=bibs&hl=en&cluster=<cid>", .open = "<", .close = ">"),
    journal = glue::glue("\\href{<cid>}{<journal> <number>}", .open = "<", .close = ">")
  ) %>%
  brief_entries(
