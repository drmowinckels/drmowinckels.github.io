...
theme_visible <- function() {
  theme_minimal(base_size = 11) +
    theme(
      ...
      axis.text = element_text(colour = visible_colours$text),
      axis.title = element_text(colour = visible_colours$text, size = 10),
      ...
      legend.text = element_text(colour = visible_colours$text),
...
```{r}
#| label: funcap-slope
#| fig-cap: "Walking, upright, home and concentration moved meaningfully between Year 1 and Year 2; hygiene, communication, outside and light stayed within sampling noise."
#| fig-alt: "Slope chart with eight FUNCAP27 categories plotted as Year 1 to Year 2 means with 95% confidence interval bars. Four lines (walking, upright, home, concentration) are drawn in coral and slope clearly upward with non-overlapping CI bars. Four lines (hygiene, communication, outside, light) are drawn in grey and barely move, with overlapping CIs."
library(ggrepel)
...
```{r}
#| label: funcap-overall
#| fig-cap: "Overall FUNCAP score across the two years. The score is the mean of all eight FUNCAP27 categories on each monthly check-in. The coral dashed line marks the rehab cutoff identified later in the post."
#| fig-alt: "Line plot of overall FUNCAP score over time, from April 2024 to April 2026. The y-axis runs from 0 to 6; the line and points are dark teal. The trajectory starts at about 3.2 in spring 2024, dips slightly through summer 2024, then climbs steadily from autumn 2024 onward, reaching the high 4s by 2026. A coral dashed vertical line in early January 2025 marks the rehab cutoff, with a 'Rehab' label."
...
#| fig-height: 7
#| fig-cap: "Two years of daily stability scores laid out as a calendar. The 2024 panel is darker and patchier; from late 2024 onwards the colour is noticeably warmer and more even, especially through 2026."
#| fig-alt: "Calendar heatmap with three rows (2024, 2025, 2026), each showing a grid of seven weekdays by week-of-year. Each tile is one day, coloured on a custom dark-teal to coral gradient by morning stability score. The 2024 panel has many dark teal and mid-tone tiles scattered through it. The 2025 panel is mostly orange and coral with occasional dark days. The 2026 panel covers only the first 17 weeks and is the warmest of the three."
stability <- visible |>
...
#| label: hrv-ridgeline
#| fig-height: 9
#| fig-cap: "Monthly distributions of morning HRV stacked as ridgelines. The mean barely moves over the two years, but the spread tightens — early-2024 distributions are wide with long right tails, and the more recent months are noticeably narrower."
#| fig-alt: "Ridgeline plot showing one density curve per month, stacked vertically from April 2024 at the bottom to April 2026 at the top. Each ridge is filled with a custom dark-teal to coral gradient based on HRV value. The early 2024 ridges are wide and lumpy with long tails extending to the right. The more recent ridges are narrower and more symmetric, sitting in roughly the same x-range."
library(ggridges)
...
#| fig-cap: "Daily morning stability score across two years, with the single change point detected by binary segmentation marked as a coral dashed line — landing right around the rehab period in early 2025."
#| fig-alt: "Scatter plot of daily stability score over time from April 2024 to April 2026, with a smoothed dark teal trend line on top. A single coral dashed vertical line is drawn at the detected change point in early January 2025. Most points cluster around 4 with some at 3 and a few at 1 or 2. The smoother trend rises slightly after the change point line."
cp_dates <- visible |>
  filter(tracker == "Stability Score") |>
  arrange(odate) |>
  slice(cpts(cp)) |>
...
#| fig-height: 5
#| fig-cap: "Daily morning HRV with four detected change points (coral dashed lines), each labelled with the timeline event it lines up with. The single stability-score change point sits in between the third and fourth HRV change points (teal dotted line)."
#| fig-alt: "Scatter plot of daily HRV from April 2024 to April 2026 with a dark teal smoothed line through the points. Four coral dashed vertical lines mark detected change points: late June 2024, mid-September 2024, mid-December 2024, and late February 2025. Above each line is a coral text label connected by a thin segment, reading 'End of bed-bound stretch', 'Pacing strategy lands', 'Run-up to rehab', and 'Post-rehab consolidation', staggered on two rows. A teal dotted vertical line marks the stability change point in early January 2025."
hrv_ts <- visible |>
  filter(tracker == "HRV") |>
  arrange(odate) |>
  pull(value)
...
#| fig-cap: "Mean morning stability score by cycle day, with the four phases shaded and labelled inline. The lowest mean sits at days 12–13 (late follicular), and the highest at days 26–28 (late luteal) — the opposite of the standard PMS-doom narrative."
#| fig-alt: "Line and point plot of mean morning stability score by menstrual cycle day, from day 1 to about day 32. Background bands are coloured by phase (Menstrual coral, Follicular orange, Ovulation mustard, Luteal teal) and labelled inline at the top of each band. A dark teal line with points traces the mean stability score, with a grey ribbon for the 95% CI. The line peaks near day 5, dips to a clear trough around day 12-13 with a coral 'Late-follicular trough' callout, and rises through the luteal phase to a peak near day 28 with a coral 'Late-luteal peak (my best days)' callout."
...
#| label: hr-cycle
#| fig-height: 7
#| fig-cap: "Resting HR and HRV by cycle day, split into pre- and post-rehab eras. Post-rehab HRV sits clearly higher and flatter; post-rehab resting HR sits clearly lower. The cyclical pattern is most pronounced in pre-rehab HRV."
#| fig-alt: "Two stacked line panels with the same x-axis (cycle day 1 to about 32). The top panel is HRV, the bottom is Resting HR. Each panel has phase shading (coral, orange, mustard, teal) labelled inline at the top of each band. Each panel also has two lines with 95% CI ribbons: a dark teal line for pre-rehab and a coral line for post-rehab. In the HRV panel, the coral post-rehab line sits clearly above the teal pre-rehab line across the cycle, with a much narrower CI ribbon. In the resting HR panel, the coral post-rehab line sits clearly below the teal pre-rehab line across the cycle. The plot subtitle notes the rehab cutoff date."
library(ggnewscale)
...
I aim to keep the _total_ daily load below a ceiling, because I've learned the hard way that an "easy" day on three axes can still leave me crashed if I went too hard on the fourth.
...
#| fig-height: 5
#| fig-cap: "Correlation between today's total activity load and stability / HRV at lags 0 to 7 days. Both lines start strongly positive at lag 0 ('active when capable') then drop toward zero, with no clear negative dip — the classic PEM signature is absent at this aggregate level."
#| fig-alt: "Line plot of correlation versus days after activity (lag 0 to 7) with two lines and 95% CI ribbons: dark teal for stability, coral for HRV. Both lines start at strongly positive correlations around 0.20 (stability) and 0.14 (HRV) at lag 0. The stability line drops sharply to about 0.01 at lag 1 then bounces between 0 and 0.08 across the rest of the lags. The HRV line stays in the 0.07 to 0.15 range across most lags. A horizontal dashed line at correlation = 0 is drawn for reference. Neither line dips clearly into negative territory."
...
What's _not_ there is a clear dip into negative territory in the 1–3 day window, which is what a PEM signature would look like.
...
On the rare days I do push past the ceiling I pay for it (and I _know_ I pay for it — see: every crash mentioned in the previous posts), but those days are rare enough that they don't move a two-year correlation much.
...
#| label: crash-windows
#| fig-height: 4
#| fig-cap: "Mean total activity load on each of the seven days before a crash, plus the crash day itself. Activity hovers around the overall baseline most of the week, spikes sharply on the day before the crash, then collapses below baseline on the crash day."
#| fig-alt: "Line plot showing mean total activity load (y-axis) by days before crash (x-axis from -7 to 0). The line is drawn in coral with a 95% CI ribbon. A grey dashed horizontal line marks the overall baseline mean of about 3.5. The activity line hovers near baseline from day -7 to day -3, dips slightly at day -3, returns to baseline at day -2, then spikes sharply to about 4.5 at day -1 (with a coral 'Day before: activity spikes' callout pointing down at the spike), then drops to about 3.0 on day 0 (with a coral 'Crash day: body pulls back' callout pointing up at the drop). A grey dotted vertical line marks day 0."
...
That's actually a more useful answer for pacing: if I notice an unusually busy day, the at-risk day is _tomorrow_, not "some time in the next three days".
