...
summary: >
  `atlas = dk` used to work. Now you need `atlas = dk()`, and that pair of
  parentheses broke a lot of people's scripts. Here is the actual reason: R
  will not let one package re-export another package's dataset, and the
...
ggseg is not one package anymore, it's a whole quite of them that we call ggsegverse[^ggsegverse].
...
That is literally what sits in ggseg's package file, and again in ggseg3d's.
...
Not aesthetics, not CRAN notes, not tidiness.
A structural wall.
...
Indeed, running `goodpractice::gp()` (which I do for most my packages), did flag using Depends like this as bad practise.
...
Goodpractise also flagged this as bad practise, and I was running out of options.

I decided to seek advice in the rOpenSci slack, where such discussions often occur and are very enlightening and wholesome.
What followed was genuinely useful, so let me walk through the options the way we actually walked through them, because every single one has a catch and I do not think any of it is obvious.

### Depends

Jon Harmon's instinct, and the conventional answer: put ggseg.formats in `Depends` rather than `Imports`, and its exports land on the user's search path when they attach ggseg.
...
It is also, notably, not what Jon himself would do.
...
Gábor's next suggestion, and this one has a nice twist.
...
I was overcomplicating it, and said so.
...
I took it to the rOpenSci Slack with the question "this works, but does it smell?", which is I think the correct question to ask about code like this.

Gábor Csárdi's reply was four words: "That `.onLoad` definitely smells bad."
...
Here is ggsegShen, doing precisely what ggseg.formats does:
...
It's not _necessary_ for all the atlas packages, really only for the default atlases shipped thorugh ggseg.formats. 
...
R has two places to put things.
`data/` is lazily loaded, documented as data, and reachable with `::` — but it sits outside the namespace, so no other package can adopt it.
The namespace is where functions live, and anything there can be re-exported, imported, documented and passed between packages freely.
