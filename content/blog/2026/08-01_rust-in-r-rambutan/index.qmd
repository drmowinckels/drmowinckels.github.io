...
It is in the general [CRAN Repository Policy](https://cran.r-project.org/web/packages/policies.html): _"Source package tarballs should if possible not exceed 10MB."_
...
R `polars`, the Rust-backed DataFrame library, _did_ make it onto CRAN.
...
I decided to stay off CRAN _first_.
...
A rambutan (_Nephelium lappaceum_) and a lychee (_Litchi chinensis_) are both in the [soapberry family, Sapindaceae](https://en.wikipedia.org/wiki/Rambutan) — same family, spikier packaging.
That was that.

![The rambutan R package hex logo. Three spiky rambutan fruits in shades of
  maroon and dusty pink sit inside a pale pink hexagon; the largest is
  half-peeled to reveal a smiling white face. The word "rambutan" runs across
  the bottom in rounded dark red letters.](featured.png)
...
Vendoring solves _offline_ builds: it bundles the crate sources so the build doesn't need the network.
