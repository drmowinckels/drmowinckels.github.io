---
title: Plotting GAMM results with ggplot2
author: DrMowinckels
date: '2019-09-07'
slug: plotting-gamm-results-with-ggplot2
categories: [R]
tags: [GAMM] 
featured_image: '/blog/2019-09-07-plotting-gamm-results-with-ggplot2_files/figure-html/featured-1.png'
---
...
```{r}
diamonds %>% 
  gather(variable, value, carat, depth) %>% 
...
There's not much fun happening with `depth` though there is this one cut that seems to behave _very_ oddly. 
We will leare depth as a main contributor, and focus on carat. 

I'd say both the carat-clarity and carat-cut plots might have some interesting interactions, so I'll dig into one of those. I'm just going to pick clarity, for the clarity (award for horrible word-puns goes to me!). 
...
But let's be honest, it aint pretty. 
It's neat for getting a quick look at it, but if you're prepping for a paper or a report, you'll need to fix it up a bit.
Let's ggplot it.
...
We can then use that data and predict what the price would be (the fit) given the model we have created.
We also want the confidence bands, so we make sure to tell the `predict` function that we want them too, by secifying the `se.fit` argument as `TRUE`.
...
For regression lines you'll not need too many points to create a good smooth line.
...
The results here indicate that the different cuts do alter the price.
