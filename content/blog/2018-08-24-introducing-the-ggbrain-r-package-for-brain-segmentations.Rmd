---
title: Introducing the ggbrain R-package for brain segmentations
author: DrMowinckels
date: '2018-08-24'
slug: introducing-the-ggbrain-r-package-for-brain-segmentations
categories:
  - R
  - Neuroimaging
tags:
  - Plots
  - Packages
featured_image: '/blog/2018-08-24-introducing-the-ggbrain-r-package-for-brain-segmentations_files/figure-html/featured-1.png'
--- 
...
The [ggbrain](https://github.com/LCBC-UiO/ggbrain) package is intended for those who need to easily plot results or overviews of segmentet data. 
In neuroimaging, we often run analyses that segment the brain into different areas, according to different types of classification atlases. 
We have, to begin with, focused on two atlases from the popular software [Freesurfer](https://surfer.nmr.mgh.harvard.edu/), the Desikan-Killany cortical atlas (dkt), and the subcortical segmentation atlas (aseg).
...
It can give you, per MRI image, mean values of area and thickness for each region in the atlas, that you can use with your hearts delight for analyses.
...
```{r, eval=F}
devtools::install_github("LCBC-UiO/ggbrain", build_vignette=T)
```

Once installed, you can have a look through the vignette with `vignette(ggbrain)`, but I'll cover some basics here.

`ggbrain` is made using `ggplot2`. Who doesn't love ggplot. It is sleek and beautiful, and we wanted these plots to be compatible with ggplot functions, like `facet_wrap` and `theme`.
Out-of-the box, calling ggbrain directly will plot the dkt atlas.
...
The atlases all have a variable called `area`, which will colour different sections in meaningful ways.
You do this with ggplot `aes` mapping.

```{r}
ggbrain(mapping=aes(fill=area), position="stacked")
```

But there are so many labels! We can't fit them all for this atlas. We can turn them off!.

```{r "featured"}
ggbrain(mapping=aes(fill=area), position="stacked", show.legend = F) +
...
ggbrain(atlas="yeo7",position="stacked",mapping=aes(fill=area),) +
...
Lets for instance say you have found that the thickness of the insula and superior frontal cortex increase with age, and you want to plot the estimated means highlighting those two areas only.

```{r}
results = data.frame(cbind(area=c("superior frontal","insula"),em=c(2.4,3.3)),
                     stringsAsFactors=F)

results %>% 
  ggbrain(mapping=aes(fill=as.numeric(em)), position="stacked") 
```
...
```{r}
results = data.frame(cbind(area=c("superior frontal","insula"),
                           em=c(2.4,3.3),
                           hemi=c("left","right")),
                     stringsAsFactors=F)
...
```{r}
results = data.frame(cbind(area=c("superior frontal","insula", "par orbitalis","pre central"),
