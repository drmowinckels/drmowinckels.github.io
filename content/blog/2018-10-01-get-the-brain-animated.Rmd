...
In all of this, I learned so much about creating packages, and ended up making quite a major alteration to the base-functions of the package. I had not completely understood the use og the ellipsis (`...`) and as such had not used it correctly. Once I started understanding it, we could make the package working closer to what a standard ggplot package would work!
...
Let's fabricate some data to work with. I want to plot the Desikan-Killany atlas, with information for mean values of thickness across different ages. Since I'm completely making all this data up, I'll just start with the `dkt` data.frame that comes with the package, grabs its unique instances of area and hemisphere (which gives me every area in the dkt atlas), and start making up some cortical thickness values per age.
...
```{r}
new_data %>% 
  ggseg(mapping=aes(fill=thickness), position="stacked", colour="darkgrey", size=.1) +
...
Now we should have a data frame with values for each area and hemisphere across the ages 5-40. 
...
To manage this, we need to go back a step when we made the data.frame, and merge our fabricated data with the entire `dkt` atlas.
...
# apply through the list to fabricate data per age, and add dkt atlas
full_data = lapply(full_data, 
       function(x)  
         base_atlas %>% 
...
You see how we've been informed that the data have been joined by `hemi` and `area`, which is exactly what we want. Let's see what it looks like now.
...
Even more crazy! But again, this is expected behaviour. You see, we have supplied an entire atlas of information to ggseg, which again tries to mege it with (in this case)  the dkt atlas. That makes this odd behaviour.

To plot this as we want, we need to tell `ggseg` that we are supplying a new `atlas` **not** new data.

```{r}
ggseg(atlas=full_data, mapping=aes(fill=thickness), position="stacked", colour="darkgrey", size=.1) +
  scale_fill_gradientn(colours=c("dodgerblue4","light blue","firebrick","goldenrod")) +
  facet_wrap(~Age)
```
...
t = ggseg(atlas=full_data, mapping=aes(fill=thickness), position="stacked", colour="darkgrey", size=.1) +
  scale_fill_gradientn(colours=c("dodgerblue4","light blue","firebrick","goldenrod")) +
...
```{r echo=F}
knitr::include_graphics("/blog/2018-10-01-get-the-brain-animated_files/figure-html/featured-1.gif")
```
